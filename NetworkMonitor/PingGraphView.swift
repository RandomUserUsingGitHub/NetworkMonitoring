...
struct PingGraphView: View {
    ...
    let theme:     AppTheme
    let maxPoints: Int
    // Injected thresholds so graph reacts to settings changes
    var thresholdGood: Double = 80
    var thresholdWarn: Double = 200

    private var scaleCeil: Double {
        let peak = data.compactMap{$0}.max() ?? 0
    ...
    private func colorFor(_ v: Double) -> Color {
        v < thresholdGood ? theme.graphOk : v < thresholdWarn ? theme.graphMid : theme.graphBad
    }
    ...
    var body: some View {
        ...
        // Height is fixed; width fills parent naturally — no explicit width here
        .frame(height: 90)
    }
    ...
    private func slotW(_ w: CGFloat) -> CGFloat {
        guard maxPoints > 0 else { return 1 }
        return (w - padL - padR) / CGFloat(maxPoints)
    }
    ...
    private func gridLayer(size: CGSize) -> some View {
        ZStack(alignment: .topLeading) {
            ForEach([0.25,0.5,0.75,1.0] as [Double], id:\.self) { frac in
                ...
                Path { p in p.move(to:CGPoint(x:padL,y:yy)); p.addLine(to:CGPoint(x:size.width-padR,y:yy)) }
        ...
        ZStack {
            ForEach(0..<max(0,data.count-1), id:\.self) { i in
                if let v0=data[i], let v1=data[i+1] {
                    ...
                    .stroke(colorFor((v0+v1)/2), style:StrokeStyle(lineWidth:2,lineCap:.round,lineJoin:.round))
                }
            }
        }
    }
    ...
    private func timeoutLayer(size: CGSize) -> some View {
        ZStack {
            ForEach(0..<data.count, id:\.self) { i in
                if data[i] == nil {
                    Text("✕").font(.system(size:9,weight:.bold)).foregroundStyle(theme.offline)
                        .position(x:xFor(index:i,width:size.width),y:padT+graphH(size.height)-6)
                }
            }
        }
    }
    ...
    private func dotLayer(size: CGSize) -> some View {
        if let li=(0..<data.count).reversed().first(where:{data[$0] != nil}), let val=data[li] {
            let cx=xFor(index:li,width:size.width), cy=yFor(value:val,height:size.height)
    ...
    private func validPoints(size:CGSize)->[(CGFloat,CGFloat)] {
        (0..<data.count).compactMap{i in guard let v=data[i] else{return nil}; return (xFor(index:i,width:size.width),yFor(value:v,height:size.height))}
    }
}
